...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/cloudflare-connection", { cache:"no-store" })
      ...
      .then((value: CloudflareInfo | null) => { if (value) { setCloudflareInfo(value); setConnected(Boolean(value.configured)); } })
      .catch(() => undefined);
  }, [authStatus.authenticated]);

  // FIX14: production data bootstraps automatically in /api/auth/status. The
  // legacy D1 migration is no longer a gate for opening the application.
  const legacyMigrationRequired = false;
  const libraryDataReady = authStatus.authenticated;
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="storage-card">
          ...
          <small>Turso + R2 persistentes</small>
...
function SettingsV2({ connected, cloudflareInfo, supervisorInfo, mcpConfigured, authUsername, onAuthUpdated, onLogout, onConnect, onSupervisor, onMcp }: { connected: boolean; cloudflareInfo: CloudflareInfo | null; supervisorInfo: SupervisorInfo | null; mcpConfigured: boolean; authUsername:string; onAuthUpdated:(username:string)=>void; onLogout:()=>void; onConnect: () => void; onSupervisor: () => void; onMcp: () => void }) {
  ...
  return <div className="content settings"><div className="page-heading"><div><p>PREFERÊNCIAS</p><h1>Configurações</h1><span>Gerencie conexão, sincronização e comportamento da biblioteca.</span></div></div><AccessSettings username={authUsername} onUpdated={onAuthUpdated} onLogout={onLogout}/><section className="mcp-settings"><div><div className="mcp-mini">⌁</div><h3>MCP para GPT</h3><p>O MCP expõe estado, evidências e controles; o ChatGPT atua como supervisor operacional.</p></div><div className="connection-box"><span className={mcpConfigured ? "status-dot on" : "status-dot"}/><div><strong>{mcpConfigured ? "Supervisor MCP conectado" : "Servidor MCP disponível"}</strong><p>Projetos, coleta, fila de decisões, QA visual, perfis, circuit breaker e ZIP.</p></div><button onClick={onMcp}>{mcpConfigured ? "Ver conexão" : "Gerar conexão"}</button></div></section><section><div><h3>Supervisor IA — Controle via MCP</h3><p>Ligar libera controle supervisor completo para o ChatGPT. Não chama Cloudflare, Llama, Qwen nem outra API de IA.</p></div><div className="connection-box"><span className={enabled ? "status-dot on" : "status-dot"}/><div><strong>{enabled ? "ChatGPT via MCP · LIGADO" : "Supervisor · DESLIGADO"}</strong><p>{enabled ? "Controle operacional COMPLETO · QA visual externo ATIVO · configurações persistentes ATIVAS." : "Somente execução determinística. Coleta autônoma continua ativa e acumula PARA_ANALISE."}</p></div><button onClick={onSupervisor}>{enabled ? "Gerenciar" : "Ligar"}</button></div></section><section><div><h3>Infraestrutura — Turso + R2</h3><p>Turso é o banco oficial e o R2 guarda os arquivos. As credenciais do R2 ficam persistidas de forma segura.</p></div><div className="connection-box"><span className={connected || cloudflareInfo?.d1Configured ? "status-dot on" : "status-dot"}/><div><strong>{connected || cloudflareInfo?.d1Configured ? "R2 configurado" : "Não configurado"}</strong><p>{cloudflareInfo?.needsReconfigure ? "Configuração antiga precisa ser substituída." : `${connected ? `R2 ${cloudflareInfo?.bucket || "configurado"}` : "R2 pendente"} · Turso ativo`}</p></div><button onClick={onConnect}>{connected || cloudflareInfo?.d1Configured ? "Gerenciar" : "Configurar"}</button></div></section><section><div><h3>Base de produção — automática</h3><p>O schema atual e o snapshot de produção são preparados automaticamente no primeiro acesso. Não há upload manual de JSON nem etapa obrigatória de D1.</p></div><div className="connection-box"><span className="status-dot on"/><div><strong>Turso é a fonte oficial</strong><p>Bootstrap idempotente · preserva login e configuração R2 · 929 assets e histórico útil incorporados ao pacote.</p></div><button disabled>Pronto</button></div></section><section><div><h3>Coleta autônoma noturna</h3><p>Pesquisa, testa URLs, materializa, valida tecnicamente e salva candidatas em PARA_ANALISE sem depender de IA.</p></div><label className="toggle"><input type="checkbox" checked readOnly/><span/></label></section><section><div><h3>QA visual obrigatório</h3><p>MATERIALIZADO não significa APROVADO. Somente decisão visual do Supervisor MCP pode aprovar candidatas novas.</p></div><label className="toggle"><input type="checkbox" checked readOnly/><span/></label></section></div>;
}
...
function ConnectModal({ connected, info, saving, onClose, onConnect }: { connected: boolean; info: CloudflareInfo | null; saving: boolean; onClose: () => void; onConnect: (values: CloudflareForm) => void }) {
  ...
  return <div className="modal-wrap"><div className="backdrop" onClick={onClose}/><section className="modal connect-modal"><header><div><span>INFRAESTRUTURA CLOUDFLARE</span><h2>{connected ? "Gerenciar R2" : "Conectar R2"}</h2><p>O Turso já é o banco oficial. Aqui você configura apenas o armazenamento físico no Cloudflare R2.</p></div><button onClick={onClose}>×</button></header>{info?.needsReconfigure && <div className="secure-note"><span>!</span><p><b>Configuração antiga detectada</b><br/>O perfil antigo foi herdado. Os campos recuperáveis já foram preenchidos; informe apenas o segredo/token que estiver indisponível e salve para substituir sem afetar os dados.</p></div>}<div className="secure-note"><span>◇</span><p><b>Configuração persistente e criptografada</b><br/>A chave R2 fica gravada no servidor. O catálogo e os estados ficam no Turso automaticamente.</p></div><div className="form-grid"><label>ACCOUNT ID<input autoComplete="off" placeholder="Cloudflare Account ID" value={values.accountId} onChange={(event) => set("accountId", event.target.value)}/><small>Usado para derivar o endpoint do R2.</small></label><label>BUCKET R2<input autoComplete="off" placeholder="Nome do bucket" value={values.bucket} onChange={(event) => set("bucket", event.target.value)}/></label><label>ACCESS KEY ID — R2<input autoComplete="off" placeholder="R2 Access Key ID" value={values.accessKeyId} onChange={(event) => set("accessKeyId", event.target.value)}/></label><label>R2 ENDPOINT (OPCIONAL)<input autoComplete="off" placeholder="https://ACCOUNT.r2.cloudflarestorage.com" value={values.endpoint} onChange={(event) => set("endpoint", event.target.value)}/><small>Vazio = derivado automaticamente do Account ID.</small></label><label>SECRET ACCESS KEY — R2<input autoComplete="new-password" type="password" placeholder={info?.hasSecret ? "R2 Secret já salvo — deixe vazio para manter" : "R2 Secret Access Key"} value={values.secretAccessKey} onChange={(event) => set("secretAccessKey", event.target.value)}/><small>{info?.hasSecret ? "Preencha apenas se quiser trocar a chave R2." : "Necessária para ativar o bucket."}</small></label><label>API TOKEN — D1 LEGADO (OPCIONAL)<input autoComplete="new-password" type="password" placeholder={info?.hasD1Token ? "Token D1 já salvo — deixe vazio para manter" : "Cloudflare API Token com D1 Read"} value={values.d1ApiToken} onChange={(event) => set("d1ApiToken", event.target.value)}/><small>Somente para recuperação legada. A produção normal não usa D1.</small></label><label>D1 DATABASE ID LEGADO (OPCIONAL)<input autoComplete="off" placeholder="Deixe vazio para localizar automaticamente" value={values.d1DatabaseId} onChange={(event) => set("d1DatabaseId", event.target.value)}/><small>{info?.d1Configured ? `Atual: ${info.d1DatabaseName || info.d1DatabaseId}` : "Se vazio, o app procura o banco pela estrutura da Corvo Library."}</small></label><label>D1 DATABASE NAME LEGADO (OPCIONAL)<input autoComplete="off" placeholder="Nome do banco, se quiser restringir a busca" value={values.d1DatabaseName} onChange={(event) => set("d1DatabaseName", event.target.value)}/></label></div><footer><button onClick={onClose}>Cancelar</button><button className="primary" disabled={!ready || saving} onClick={() => onConnect(values)}>{saving ? "Testando e salvando..." : "Salvar e cravar configuração"}</button></footer></section></div>;
}
