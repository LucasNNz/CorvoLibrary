...
import { CloudflareAdminError, resolveCorvoD1Database } from "../../../lib/cloudflare-admin";
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    let d1Diagnostic: Record<string, unknown> | null = null;
    if (connection.d1ApiToken) {
      ...
      d1Diagnostic = { ok: true, code: "CLOUDFLARE_D1_FOUND", databaseId: database.id, databaseName: database.name };
    }
    ...
    return Response.json({ ...safeCloudflareConnection(saved), bindingActive: hasAnyR2, d1Diagnostic }, { headers });
  } catch (error) {
    if (error instanceof CloudflareAdminError) {
      const status = error.httpStatus === 401 ? 401 : error.httpStatus === 403 ? 403 : 400;
      return Response.json({
        error: error.message,
        code: error.code,
        details: error.details || "",
        d1Diagnostic: { ok: false, code: error.code, message: error.message },
      }, { status, headers });
    }
    return Response.json({
      error: error instanceof Error ? error.message : String(error),
      code: "CLOUDFLARE_CONNECTION_VALIDATION_FAILED",
    }, { status: 500, headers });
  }
}
