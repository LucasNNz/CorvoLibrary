...
import { decideProjectThumbnails, decideProjectTitles, exportCompleteProjectZip, getProjectProductionPackage, pushProjectThumbnailFileBytes, pushProjectThumbnailUrlBatch, pushProjectTitles } from "./project-production-package";
...
export const tools: Tool[] = [
  ...
  tool("configurar_modo_entrega_chat", "Configurar modo de entrega ao chat", "Define OFF (industrial/links-only, recomendado) ou ON (compatibilidade legada com resource_link). OFF nunca impede FAST PUSH FILE de receber um anexo fornecido pelo usuário; apenas bloqueia entrega automática de arquivos do R2 ao chat.", objectSchema({ modo:text("OFF ou ON") }, ["modo"]), false),
  ...
  tool("fast_decidir_candidatas_lote", "FAST DECIDE — approve/reject/relink", "Rota assíncrona compacta para até 200 decisões mistas. Cada decisão aceita candidate_id e/ou item_id/target_file + action APPROVE|REJECT|RELINK. Persiste operation_id e devolve ACK imediato; freeze, rotação de candidata, relink, contadores e fan-out seguem no Data Plane.", objectSchema({ project_id:text("Projeto"), decisions:array(objectSchema({ candidate_id:text("Candidate ID opcional"), item_id:text("PITEM/item_key opcional"), target_file:text("target_file opcional"), action:text("APPROVE, REJECT ou RELINK"), reason:text("Motivo opcional") }, ["action"]),"Até 200 decisões"), operation_id:text("Recibo idempotente"), execution_id:executionField }, ["project_id","decisions"]), false),
  ...
  tool("fast_push_thumb_arquivo", "FAST PUSH THUMB do chat", "Opção secundária quando a thumb já está fisicamente no ChatGPT. Transporta os bytes diretamente para o pacote de produção do projeto, sem PITEM e sem transformar o arquivo em nova origem web.", objectSchema({ arquivo:openAiFileSchema, contexto:productionThumbFileContextSchema }, ["arquivo","contexto"]), false, false, false, { "openai/fileParams":["arquivo"], "corvo/fastPush":true, "corvo/projectProduction":true }),
  ...
  tool("exportar_projeto_completo_zip", "Exportar projeto completo ZIP", "Gera o pacote de produção separado do ZIP operacional: ROTEIRO.txt, IMAGENS_NECESSARIAS.txt, IMAGENS/, THUMBS/, TITULOS.txt e PROJETO.json. Retorna link assinado quando a configuração R2 permite.", objectSchema({ project_id:text("Projeto central"), validade_minutos:integer("Validade do link, 1..1440") }, ["project_id"]), false),
  ...
  try {
    ...
    switch (name) {
      ...
      case "fast_decidir_candidatas_lote": {
        const projectId=string(input.project_id),operationId=string(input.operation_id)||`FAST-DECIDE-${projectId}-${Date.now().toString(36)}`,decisions=(Array.isArray(input.decisions)?input.decisions:[]).slice(0,200);
      ...
      case "importar_candidatas_url_lote": {
        result = await ingestFastPushBatch((Array.isArray(input.itens) ? input.itens : []).map((item) => record(item)), optional(input.batch_id));
        break;
      }
      case "fast_push_urls_lote": { result = await ingestFastPushBatch((Array.isArray(input.itens) ? input.itens : []).map((item) => record(item)), optional(input.batch_id)); break; }
      ...
      case "fast_push_thumb_arquivo": {
        const file = record(input.arquivo), context = record(input.contexto), downloadUrl = string(file.download_url), fileId = string(file.file_id), mimeType = string(file.mime_type).toLowerCase(), fileName = string(file.file_name) || fileId;
        if (!fileId || !downloadUrl.startsWith("https://")) throw new Error("O ChatGPT não forneceu uma referência de arquivo válida.");
        const transport = await fetch(downloadUrl);
        if (!transport.ok) throw new Error(`CHAT_FILE_TRANSPORT_HTTP_${transport.status}`);
        result = await pushProjectThumbnailFileBytes(new Uint8Array(await transport.arrayBuffer()), fileName, mimeType, { operation_id:string(context.operation_id), project_id:string(context.project_id), name:string(context.name) || fileName, variant:string(context.variant), agent_origin:string(context.agente_origem), observation:string(context.observacao), source_type:"CHAT_FILE", source_url:`chat-file://${fileId}` });
        break;
      }
      ...
      case "exportar_projeto_completo_zip": {
        const exported = await exportCompleteProjectZip(string(input.project_id)), minutes = Math.max(1, Math.min(1440, Number(input.validade_minutos) || 60));
        try { result = { ...exported, download_url: await createSignedR2GetUrl(exported.r2Key, minutes, exported.fileName, "application/zip"), validade_minutos: minutes }; }
        catch (error) { result = { ...exported, download_url: null, download_error: error instanceof Error ? error.message : "SIGNED_URL_UNAVAILABLE" }; }
        break;
      }
